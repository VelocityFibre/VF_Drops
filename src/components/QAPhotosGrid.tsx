...
export default function QAPhotosGrid() {
  ...
  const [searchDropNumber, setSearchDropNumber] = useState<string>('');
  const [filteredReviews, setFilteredReviews] = useState<QAPhotoReview[]>([]);

  useEffect(() => {
    fetchQAReviews();
  }, []);

  // Filter reviews based on user and drop number search
  useEffect(() => {
    let filtered = qaReviews;
    
    // Filter by selected user
    if (selectedUser) {
      filtered = filtered.filter(review => review.user_name === selectedUser);
    }
    
    // Filter by drop number search
    if (searchDropNumber.trim()) {
      const searchTerm = searchDropNumber.trim().toLowerCase();
      filtered = filtered.filter(review => 
        review.drop_number.toLowerCase().includes(searchTerm)
      );
    }
    
    setFilteredReviews(filtered);
  }, [qaReviews, selectedUser, searchDropNumber]);
  ...
  const displayReviews = filteredReviews.length > 0 || searchDropNumber.trim() || selectedUser ? filteredReviews : qaReviews;
  const totalCompleted = displayReviews.reduce((sum, review) => sum + review.completed_photos, 0);
  const totalOutstanding = displayReviews.reduce((sum, review) => sum + review.outstanding_photos, 0);
      ...
      <div className="mb-4">
        ...
        <div className="flex justify-between items-center mb-4">
          <div className="flex gap-4 items-center">
            <h2 className="text-xl font-semibold">
              QA Photos Review ({displayReviews.length} of {qaReviews.length} drops)
            </h2>
        ...
        <div className="flex gap-3 items-center">
          <div className="flex-1">
            <input 
              type="text"
              placeholder="Search by Drop Number (e.g., DR1751191)..."
              value={searchDropNumber}
              onChange={(e) => setSearchDropNumber(e.target.value)}
          ...
          {(searchDropNumber.trim() || selectedUser) && (
            <button 
